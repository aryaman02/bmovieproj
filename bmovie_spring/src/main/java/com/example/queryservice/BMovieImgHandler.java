...
public class BMovieImgHandler extends BMovieImgHandlerGrpc.BMovieImgHandlerImplBase {
    private BMovieQueryServiceDriver bmService;

    public BMovieImgHandler(BMovieQueryServiceDriver bmqsd) {
        bmService = bmqsd;
    }

    public void getImgOfBMovie(com.example.demo.remote.BMovieImgRequest request,
        io.grpc.stub.StreamObserver<com.example.demo.remote.BMovieImgResponse> responseObserver) {

        BMovieImgResponse.Builder builder = BMovieImgResponse.newBuilder();
    ...
    private void processClientRequest(String imdbID, BMovieImgResponse.Builder builder) throws IOException {
        ...
        if (query.size() > 0) {
            ...
            if (bMovieDescription != null) {
                builder.putResponse(imdbID, QueriedResultsGRPC.newBuilder().setFlag(true).addImdbIDs(imdbID).build());
            ...
            if (bMovieInfo.equals("{\"Response\":\"False\",\"Error\":\"Error getting data.\"}")) {
                builder.putResponse(imdbID, QueriedResultsGRPC.newBuilder().setFlag(false).build());
                return;
            ...
            if (bMovieInfo.indexOf("\"Type\":\"movie\"") != -1) {
                ...
                if (bMovieInfo.indexOf("\"Country\":\"India\"") != -1 && bMovieInfo.indexOf("\"Language\":\"Hindi\"") != -1 && year >= 1990 && year <= 2009) {
                    ...
                    builder.putResponse(imdbID, QueriedResultsGRPC.newBuilder().setFlag(true).addAllImdbIDs(emptyList).build());
                    return;
                }
            } else {
                List<String> emptyList = new ArrayList<>();
